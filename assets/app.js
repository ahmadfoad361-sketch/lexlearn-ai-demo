...
function header(){
  ...
  if(a&&a.role==="admin"){
    nav='<button class="iconbtn" data-admin-home="1">👥 لوحة الإدارة</button><span class="pill">مدير المنصة</span><button class="iconbtn" id="logoutBtn">العودة</button>';
    ...
    nav+='<span class="pill">'+esc(a.name)+'</span>'+(s&&s.completed?'<span class="pill gold">⭐ '+s.xp+' XP</span><span class="pill">🔥 '+s.streak+'</span>':'')+'<button class="iconbtn" id="logoutBtn">العودة</button>';
  }
...
function courseScreen(){
  ...
  return '<section class="panel screen onboarding">'+journey("course")+'<div class="screenhead"><div><div class="kicker">اختيار المقرر</div><h2>اختر المقرر الذي تريد دراسته</h2><p class="small">كل مقرر له تشخيصه وأسئلته ونتيجته ومساره المستقل.</p></div><span class="stepbadge">🇶🇦 جامعة قطر</span></div><div class="grid2">'+courses.map(function(c){
...
function render(){
  ...
  html+='</main><div class="footer">LexLearn AI • تعلم قانوني تكيفي • v6 Prototype</div><div id="toast" class="toast"></div>';
